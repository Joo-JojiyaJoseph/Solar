                ...
                <tbody>
                    @foreach ($leads as $lead)
                        <tr @if ($lead->status == 'pending') style="background: #9d1a1a;" @endif>
                            ...
                            <td>{{ $lead->contact_number }}<br>{{ $lead->alternate_number }}</br>{{ $lead->alternate_number1 }}
                            </td>
                            <td>{{ $lead->email }}</td>
                                ...
                                <td>
                                    @if ($user->type == 'admin' || $user->type != 'branch')
                                        <button type="button" class="btn btn-primary btn-block mb-2"
                                            data-toggle="modal" data-target="#edit{{ $lead->id }}">Edit</button>

                                        <a class="delete_btn btn btn-danger btn-block"
                                            data-action="{{ $lead->id }}" message="Delete the $lead">
                                            Delete
                                        </a>
                        ...
                        <div class="modal-content">
                            ...
                            <form action="{{ route('lead.update', $lead_edit) }}" method="post"
                                    ...
                                    <select name="branch"
                                        ...
                                        @foreach ($branchs as $branch)
                                            <option value="{{ $branch->id }}"
                                                {{ $branch->id == $lead_edit->branch ? 'selected' : '' }}>
                                                {{ $branch->name }}</option>
                                        @endforeach
                                    </select>
                                </div>
                                <div>
                                    ...
                                    <input type="text" name="customer_name" value={{ $lead_edit->customer_name }}
                                        required
                                        class="w-full px-3 text-black py-2 mt-1 border rounded-lg focus:ring-2 focus:ring-blue-400 transition duration-300">
                                </div>
                                ...
                                <div>
                                    ...
                                    <input type="tel" name="contact_number"
                                        value={{ $lead_edit->contact_number }} required
                                        class="w-full px-3 text-black py-2 mt-1 border rounded-lg focus:ring-2 focus:ring-blue-400 transition duration-300">
                                </div>
                                    ...
                                    <select name="service" id="service"
                                        ...
                                        @foreach ($services as $service)
                                            <option value="{{ $service->id }}"
                                                {{ $service->id == $lead_edit->service ? 'selected' : '' }}>
                                                {{ $service->name }}</option>
                                        @endforeach
                                    </select>
                                </div>
